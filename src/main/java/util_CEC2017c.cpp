...
extern int ini_flag,n_flag,func_flag,*SS,dimension,fun;

double *OShift,*M,*y,*z,*x_bound;
int ini_flag=0,n_flag,func_flag,*SS,dimension,fun;

JNIEXPORT void JNICALL Java_util_CEC2017c_init
  (JNIEnv *env, jobject obj, jint dim, jint func) {
...
JNIEXPORT jdouble JNICALL Java_util_CEC2017c_fitness
  (JNIEnv *env, jobject obj, jdoubleArray arr) {
	...
	f=(double *)malloc(sizeof(double));
        ...
        cec17_test_func(x, f, dimension,1,fun);
...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "CEC2017input_data/M_%d_D%d.txt", func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "CEC2017input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
		...
		{
			sprintf(FileName, "CEC2017input_data/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
