 ...
 * 
 * @author wikki on 02/08/2019
 */
public class CuttingStock1D implements TestFunction {
    ...
    public CuttingStock1D() {
    }

    public CuttingStock1D(int[][] order, int[] stock, int cut_through) {
...
//                    real_used_rods = new int[]{single_used_rods[0], this.getIndex(vector[i], single_used_rods[1]-1)+1};
...
//        System.out.println("Waste: " + this.waste);
        ...
        return rodsU + this.countLeftToCut() + this.waste;
    }
    ...
    protected int[] countUsedRods(List<Integer> pattern, int rod_index) {
        ...
        for(Integer order_index : pattern) {
            ...
            pattern_count = 0;
            pattern_count = pattern.stream().filter((order_index2) -> (Objects.equals(order_index, order_index2))).map((_item) -> 1).reduce(pattern_count, Integer::sum);
...
//                if (!(pattern.contains(i) && order1[1] == 1)) {
//                    possibilities.add(i);
//                }
            }
        }
    ...
    public HashMap<Integer, Integer> getStock_used() {
        return stock_used;
    }
    ...
    public static void main(String[] args) {
        ...
        cut = new CuttingStock1D(order, stock, cut_through);
        ...
        cut.fitness(trial);
//        System.out.println("Fitness: " + cut.fitness(trial));
...
//        System.out.println("Fitness: " + cut.fitness(trial));
        ...
        System.out.println("Used rods: " + cut.fitness(trial));
        
        System.out.println(cut.writeDownPatterns());
        
//        for(int i = 0; i < 100; i++) {
//            trial = cut.generateTrial(200);
//            System.out.println("Used rods: " + cut.fitness(trial));
//        }
