 ...
 * the demand may be partially satisfied
 * All nodes can be visited multiple times and the path is not continuous
 */
public class Network4 implements TestFunction {
    ...
    public double fitness(double[] vector) {
        ...
        Integer[] path_point;
        int next_node = -1, amount = 0, cur_node = -1, last_index = 0, max_wanted, last_start = -1, cumulative_amount = 0, available_amount;
        ...
        for(int i=0; i<vector.length; i++){
            ...
            //select sell node
                ...
                available_amount = Math.abs(amount);
                if(available_amount >= X[next_node]){
                    amount += X[next_node];
                    ...
                    nodeLoad[next_node] += available_amount;
                    cur_path.add(new Integer[]{cur_node, next_node});
                    cur_amounts.add(available_amount);
                    X[next_node] -= available_amount; 
                }
                cur_node = next_node;
