 ...
 * No way back
 * Do not need to sell everything
 * Single starting position
 */
public class Network2b implements TestFunction {
...
//        amount = X[start];
        amount = this.getAmountFromFirstFeature(vector[0], X[start]);
//        X[start] = 0;
        X[start] = X[start] + amount;
    ...
    /**
     * Uses first feature of the individual to select original amount.
     * 
     * @param feature
     * @param maxAmount
     * @return 
     */
    public int getAmountFromFirstFeature(double feature, int maxAmount){
        
        double normalizedFeature = (feature - this.min(0)) / (this.max(0) - this.min(0));
        int actAmount = (int) Math.round(normalizedFeature * (Math.abs(maxAmount)));
        
        this.actualAmount = actAmount;
