 ...
 */
public class AP_ann_DISH implements Algorithm {
...
//            }
            ...
            this.isBest(ind);
            this.P.add(ind);
            ind.equation = ((APtfann) f).ap.equation;
//            ind.length = ((APtfann) f).countLength(features);
            this.FES++;
            this.writeHistory();
        }
    ...
    public AP_ann_DISH(int D, int MAXFES, TestFunction f, int H, int NP, Random rndGenerator, int minPopSize) {
    ...
    public AP_Individual run() {
        ...
        while (true) {
            ...
            for (int i = 0; i < this.NP; i++) {
                ...
                trial.equation = ((APtfann) f).ap.equation;
//                trial.length = ((APtfann) f).countLength(u);
