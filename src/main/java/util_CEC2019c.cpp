...
int ini_flag=0,n_flag,func_flag,*SS,fun,dimension;
...
void cec19_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		if (func_num > 3)
		{
		sprintf(FileName, "CEC2019input_data/M_%d_D%d.txt", func_num,nx);
		...
		if (func_num > 3)
		{
		sprintf(FileName, "CEC2019input_data/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
